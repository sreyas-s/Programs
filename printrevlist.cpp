...
int main(){
	...
	int ch,n,i;
	while(1){
		cout<<"\n Enter the choice:\n 1. add at position \n 2. delete from position \n 3. value at position \n 4. display \n 5. print reverse of the linked list \n 6. Exit \n";
		cin>>ch;
		switch(ch){
...
void printrevfun(struct node **headref){
	//have to write the base condition for stopping the recursion here.
	struct node *temp=*headref;
	if(temp!=NULL){
		printrevfun(&(temp->next));
		cout<<temp->data<<" ";
		}
	else{
		return;
